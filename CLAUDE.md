...
| `programacion_aa.py`, `PROGRAMACION_AA.bat` | **Hoja de inventario** de cada reinicio del ciclo Bod→BodFarm (cada 14 días desde `pedido_fusion.CICLO_INICIO`): `Programacion_AA/Programacion_AA_<fecha>.xlsx` (programado/solicitado SSASUR vs Stock Bodega AA + columna Stock Real para el conteo físico), luego `--aplicar-conteo` → `Resumen_Programacion_AA_*.xlsx`. AUTO_SSASUR la genera sola (PASO 5g, `--auto`): una vez por ciclo, con recuperación hasta 7 días si el primer día hábil no corrió, y solo con reporte de Programación bajado ese día. La planilla sale en `.xlsx` y en `.pdf`, ambos en **carta vertical** para imprimir: el PDF se hace con reportlab (`_escribir_pdf`) y trae encabezado repetido y la columna "Stock Real" en blanco. `publicar_escritorio.py` (`sync_inventario`, `--inventario`) copia xlsx, pdf y Resumen del ciclo actual a `Escritorio\Farmacia AA\8 - Inventario Bodega AA\` y quita los de ciclos anteriores. `publicar_drive.py` (`sync_inventario`, `--solo-inventario`) los sube a `Farmacia AA/7 - Inventario Bodega AA/`, pero SOLO si no están y nunca los actualiza. El xlsx convertido a Sheets no tiene md5, así que se re-subiría a diario y pisaría un conteo anotado en el Sheet. Eliminado 04-09-2026 y **restaurado 14-09-2026** porque la hoja se sigue usando |
...
**Eliminados 04-09-2026** (dashboard Streamlit + auditorías/programación sueltas, fuera de las 6 categorías que el usuario pidió conservar): `app_pedidos.py`, `app_maestro.py` (+ `paginas/*.py` y `estilo_maestro.py`, exclusivos del hub), `agente_duplicados.py`, `auditoria_medicamento.py`, `auditoria_duplicados_profunda.py`, `auditoria_prescripcion.py`, `auditoria_cantidad_posologia.py`, `auditoria_insulinas.py`, `app_auditoria_retiros.py`, `programacion_aa.py` (restaurado 14-09-2026, ver tabla), `actualizar_programacion_sept.py`, `_generar_glosario.py`, `crear_acta_vencimiento.py`, `subir_prueba_sheets.py`. No reintroducir sin confirmar con el usuario (mismo criterio que otras eliminaciones — ver memoria del proyecto).
...
- **Reporte de Programación AA (PASO 4b de `AUTO_SSASUR.py`)**: se baja TODOS los días (mes en curso) y lo consume `programacion_aa.py`. No agregar `--no-programacion` a `AUTO_SSASUR.bat`. El 14-09-2026 se apagó por creerlo sin uso y ese mismo día hubo que reactivarlo, porque la hoja de inventario del reinicio de ciclo lo necesita.
- **Inicio de ciclo en `programacion_aa.py`**: es el primer día hábil desde el lunes de cada ventana de 14 días (`CICLO_INICIO`), NO `_dias_ciclo() == 10`. Un feriado dentro del ciclo lo deja con menos de 10 días hábiles, y con la regla vieja la planilla no se reconocía como inicio (caso 14-09-2026, feriado 18-09).
- **Drive/Escritorio recortados 04-09-2026**: `publicar_drive.py` y `publicar_escritorio.py` solo sincronizan las 6 categorías vigentes (Fusión AA, Gestión Territorial, Controlados, Clozapinas, Centinela Invierno/SM, Servicios Farmacéuticos) + infraestructura. Las carpetas "App Pedidos", "Auditoria Prescripcion" y "Programacion AA" (en Drive y en Escritorio\Farmacia AA\) se eliminaron — no reintroducir sin confirmar. Excepción confirmada por el usuario el 14-09-2026: la hoja de inventario del ciclo vuelve a publicarse, en "8 - Inventario Bodega AA" (Escritorio) y en "7 - Inventario Bodega AA" (Drive).

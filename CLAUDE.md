...
| `conteo_controlados_app.py` | App Streamlit (puerto 8503) del conteo de controlados — 1 computador, historial en localStorage: sirve `conteo_controlados.html` vía `components.html`, inyecta config + stock |
| `conteo_controlados.html` | Formulario de conteo (3 farmacias: AT Cerrada/Abierta/Urgencia; exportar JSON/Excel/PDF). Corre suelto, vía la app Streamlit, o vía `servidor_conteo.py` (modo online: `window.CONTEO_ONLINE` activa el sync con el servidor) |
| `servidor_conteo.py` | Servidor de RED (puerto 8504, stdlib `http.server`, sin dependencias nuevas) para llenar el conteo desde **varias computadoras a la vez** (1 persona por farmacia). Estado compartido en `Conteo_Controlados/estado_compartido.json`; historial vía endpoints atómicos agregar/eliminar. `SERVIDOR_CONTEO_ONLINE.bat` (red del hospital) |
| `servidor_conteo_internet.py` | Igual que `servidor_conteo.py` pero además abre un túnel de Cloudflare (`cloudflared`, quick tunnel) → accesible por **INTERNET** desde cualquier red, no solo la del hospital. **SIN clave, a propósito** (decisión del usuario 24/25-07). URL cambia en cada reinicio del túnel (no hay dominio propio); se guarda en `Conteo_Controlados/url_publica.txt`. Reintenta solo si cloudflared cae, pero NO arranca solo al reiniciar el PC (sin tarea programada, a pedido del usuario). `SERVIDOR_CONTEO_INTERNET.bat` |
| `stock_controlados.py` | Baja el stock de sistema de AT Abierta/Cerrada → `Conteo_Controlados/stock_sistema.json` (pre-llena columna Sistema). Cada bodega usa su reporte y fecha (ver `controlados_config.json` → `fuente_stock`/`dia_stock`). Lo llama AUTO_SSASUR (PASO 4d) |

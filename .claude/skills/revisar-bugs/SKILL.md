...
| 2026-09-07 | publicar_drive.py | **Causa raíz real** (no el PASO 5c2, que ya estaba corregido): `_depositar_arbol_local()` — cuando 2 rangos GT distintos (`out_gt/<rango1>`, `out_gt/<rango2>`) generaban una Planilla/Letrero *distinta* para el mismo destino el mismo día (ej. 04-09-2026: `03-09-2026_04-09-2026` y `03-09-2026_17-09-2026`, este último con el margen +13 días viejo — ver fix de AUTO_SSASUR ese mismo día), la función NO fusionaba: aparcaba el segundo archivo bajo un sufijo `(rango <carpeta>)` (línea `dest = os.path.join(ddir, f"{base} (rango {...}){ext}")`), dejando 2 nóminas sueltas — CESFAM TEODORO SCHMIDT con 61+20 pacientes (0 en común, 61 ausentes si se usó el archivo más nuevo) y CESFAM QUEPE con 1+1. Un tercer reporte (`reporteGestionTerritorial_03-09-2026_17-09-2026.xlsx`) ni siquiera había llegado a fusionarse — sus 9 recetas de Teodoro Schmidt seguían totalmente ausentes hasta que el usuario las señaló a mano el 07-09-2026 comparando contra el reporte crudo. | ✅ Corregido: `_depositar_arbol_local()` ahora fusiona por N° Receta (`_fusionar_en_deposito()`, misma lógica de `_leer_nomina_existente`+`generar.hoja_funcionarios` que ya usaba `fusionar_nominas_gt.py`) en vez de aparcar un archivo `(rango ...)` separado — nunca debe volver a quedar más de 1 Planilla/Letrero oficial por destino/día. El caso puntual del 04-09-2026 se reparó a mano (90 pacientes finales en Teodoro Schmidt, 4 en Quepe) antes de aplicar el fix de código. Probado con una simulación de 2 rangos con recetas disjuntas — el resultado queda fusionado sin archivos "(rango...)". |
